...
public class FloatingWindowService extends Service
{
    ...
    public int onStartCommand(Intent intent, int flags, int startId)
    {
        ...
        {
            Log.d("启动", "悬浮窗");
            showFloatingWindow(); // 创建悬浮窗初始化子视图
            updateVisibility(); // 更新悬浮窗子视图可见性
            startUpdatingMetrics(); // 开始更新网速
...
//    使用 BroadcastReceiver监听屏幕方向变化
    private final BroadcastReceiver broadcastReceiver = new BroadcastReceiver()
    {
        @Override
        public void onReceive(Context context, Intent intent)
        {
            if (intent != null && intent.getAction() != null && intent.getAction().equals(Intent.ACTION_CONFIGURATION_CHANGED))
            {
                ...
                } else if (config.orientation == Configuration.ORIENTATION_PORTRAIT)
                {
                    ...
                    if (!isFloatingViewVisible)
                    {
                        ...
                        windowManager.addView(floatingView, params); // 创建悬浮窗
                        n = 0;
                        startUpdatingMetrics(); // 开始更新网速
                    }
                }
    ...
    private void updateMetrics()
    {
        ...
        Log.d("更新", String.valueOf(uploadSpeed));
    ...
    public void changeMovable(boolean movable)
    {
        ...
        {
            Log.d("changeMovable", "出错" + isMovable);
        }
    }
